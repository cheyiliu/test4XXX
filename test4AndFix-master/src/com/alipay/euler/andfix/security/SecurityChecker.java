...
public class SecurityChecker {
	...
	public boolean verifyOpt(File file) {
	    Log.d("euler", "SecurityChecker, verifyOpt");
	...
	public void saveOptSig(File file) {
	    Log.d("euler", "SecurityChecker, saveOptSig");
		String fingerprint = getFileMD5(file);
	...
	public boolean verifyApk(File path) {
	    Log.d("euler", "SecurityChecker, verifyApk");
		if (mDebuggable) {
	...
	private void loadDigestes(JarFile jarFile, JarEntry je) throws IOException {
	    Log.d("euler", "SecurityChecker, loadDigestes");
	    InputStream is = null;
		try {
	...
	private boolean check(File path, Certificate[] certs) {
	    Log.d("euler", "SecurityChecker, check");
		if (certs.length > 0) {
	...
	private String getFileMD5(File file) {
	    Log.d("euler", "SecurityChecker, getFileMD5");
		if (!file.isFile()) {
			return null;
		}
	...
	private void saveFingerprint(String fileName, String md5) {
	    Log.d("euler", "SecurityChecker, saveFingerprint");
	...
	private String getFingerprint(String fileName) {
	    Log.d("euler", "SecurityChecker, getFingerprint");
		SharedPreferences sharedPreferences = mContext.getSharedPreferences(
				SP_NAME, Context.MODE_PRIVATE);
	...
	private void init(Context context) {
	    Log.d("euler", "SecurityChecker, init");
		try {
